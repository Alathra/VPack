...
public class PackInfo {
    ...
    public PackInfo(
        ...
        if (urlString == null){
            plugin.getLogger().warn("[BSP] Missing config key \"pack-uri\"");
            urlString = "";
        }
        ...
        if (setHash && this.url != null){
            try{
                ...
                plugin.getLogger()
                        .warn("[BSP] Resourcepack inaccessible. You will need to reload the hash by restarting the proxy");
            }
        }
    }
    ...
    public void setUrl(URL url) {
        this.url = url;
        Settings.setPackUrl(url == null ? "" : url.toString());
    }
    ...
    private byte[] fetchSha1() throws IOException{
        ...
        try{
            plugin.getLogger().info("Fetching resource pack hash");
        ...
        plugin.getLogger().info("Downloading pack from {}", this.getUrl());
        InputStream packStream = this.getPack();
        plugin.getLogger().info("Pack downloaded, assembling hash and exiting");
